...
// job application statuses on which user can not apply
export const NON_WITHDRAW_STATUSES = [
  'Rejected',
  'Offer Withdrawn',
  'To Be Offered',
  'Offer Accepted',
...
export const determineJobTag = (jobData, eligibilityCriteria) => {
  ...
  const isExpired =
    expiry && (new Date(expiry) < new Date() || jobProfileStatus !== 'active');
  ...
  // Priority 3: Experience Ineligible
  ...
  // Priority 4: Notice Period Mismatch
  if (noticePeriodReason && !noticePeriodReason.isEligible) {
    const allOtherMet = checkAllOtherRequirementsMet(
      reasonsMap,
      'notice_period'
    );
  ...
  // Fallback: Ineligible
  return {
    tag: ELIGIBILITY_TYPES.ineligible,
    text: 'Ineligible',
    icon: 'CloseCircleOutlined',
    color: '#a42137',
    antdColor: 'error',
